...
# You can set warning and critical \
# integer/precentage(%) values \
# with -w and -c. With -p you can output\
# performance data and with 'swap' keyword generate alerts\
# based on swap usage\
#\
# Install:\
...
#\
SWAP_TOTAL=$($\{TOP\} -b |$\{GREP\} ^Swap |$\{NAWK\} '\{ print $2 \}' |$\{SED\} 's/M//g')\
SWAP_FREE=$($\{TOP\} -b |$\{GREP\} ^Swap |$\{NAWK\} '\{for(i=1;i<=NF;i++) if( $i ~ /Free/ ) print $(i-1)\}' |$\{SED\} 's/M//g')\
...
if [ "$perf" = 1 ]; then\
        ...
        OUTPUT="$OUTPUT | 'total'=$TOTAL_RAM 'used'=$USED_RAM  'active'=$ACTIVE_MEM  'inactive'=$INACT_MEM 'wired'=$WIRED_MEM 'cache'=$CACHE_MEM 'buf'=$BUF_MEM 'swap_total'=$SWAP_TOTAL 'swap_used'=SWAP_USED"\
else\
        OUTPUT="Total: "$TOTAL_RAM"M Used: "$USED_RAM"M Active: "$ACTIVE_MEM"M Inact: "$INACT_MEM"M "\
        OUTPUT="$OUTPUT Wired: "$WIRED_MEM"M Cache: "$CACHE_MEM"M Buf: "$BUF_MEM"M Swap: "$SWAP_TOTAL"M SwUsed: "$SWAP_USED"M"\
fi\
